...
* 你正在协助的对象是 **Roy**。
* 假设 Roy 是一名经验丰富的资深后端 / 数据库工程师，熟悉 Rust、Go、Python 等主流语言及其生态。
* Roy 重视“Slow is Fast”，关注点在于：推理质量、抽象与架构、长期可维护性，而不是短期速度。
* 你的核心目标：
...
1. 目标：让任意目录/文件可在 30 秒内定位 Input/Output/Position，并明确变更影响面。
2. 定义：
   - 触达：本次改动新增/删除/移动文件；或修改 public 导出/接口语义；或引入/移除关键依赖；或改变错误/权限/数据流策略。
   - 目录链路：被触达目录 + 向上直到模块根（如 frontend/ 或 server/）之间的目录集合。
   - 语义变化：对外可观察行为变化（返回/错误/副作用/权限/关键性能约束）。
3. 范围：仅对“本次改动触达的目录链路”生效；禁止全仓库扫荡式补齐。
4. 目录文档：每个被触达目录维护 CURRENT_CODE_INFO.md（定位/边界/变化触发 ≤3 行，不含标题；+ Files 文件清单：文件名-地位-功能）。
   - generated/vendor 只需标注 generated/vendor 与来源/用途一句话
   - CURRENT_CODE_INFO.md 为实验态，不入 git（忽略机制由本地配置负责）
5. 文件头：每个被触达的非生成源文件，文件开头必须有中文三行（可追加 1 行触发器）：
   - Input: ...
   - Output: ...
   - Position: ... -（可选）语义变化时同步更新以上三行与所属目录 CURRENT_CODE_INFO.md。
6. 正式文档上收：若变更属于功能/架构/对外接口/错误策略/关键依赖变化，除更新 CURRENT_CODE_INFO.md 外，还必须同步更新可入库的正式文档（优先 `.works/spec/`；必要时更新模块 README/tech doc/根 README 的相关段落）。
7. 豁免：generated/vendor 不要求补齐内容真实性；禁止为满足协议修改生成文件本体。

### CURRENT_CODE_INFO.md 模板（目录级）
...
- 定位：...
- 边界：...
- 变化触发：当本目录职责/文件清单/对外导出发生变化时，必须更新本文件。
...
// Output: ...
// Position: ...
// 语义变化时同步更新以上三行与所属目录 CURRENT_CODE_INFO.md。
```
